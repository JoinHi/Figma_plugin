...
async function handleExport(message) {
    try {
        ...
        const design = createDesignDocument(exportableRoots, rootNode, context, selection.length, previewOutcome.metadata);
...
function getExportableRoots(roots) {
...
async function createVirtualSelectionRoot(roots, context, selectedNodeCount) {
    const children = [];
    for (const root of roots) {
...
async function createPreviewOutcome(roots, rootBox) {
    try {
        if (roots.length === 1) {
            postProgress({
                phase: "preview",
...
async function exportPreviewFile(root, scale) {
...
async function createPreviewComposition(roots, rootBox, scale) {
    const parts = [];
    const warnings = [];
    let current = 0;
    for (const root of roots) {
...
function chooseMultiPreviewScale(rootBox, partCount) {
...
function createDesignDocument(roots, rootNode, context, selectedNodeCount, previewMetadata) {
...
async function serializeStyle(node, context) {
    const style = removeEmpty({
...
async function serializePaints(value, context) {
...
async function serializePaint(paint, context) {
    const source = asObject(paint);
    const type = String(source.type || "UNKNOWN");
    const base = removeEmpty({
        type,
...
async function registerImageAsset(imageHash, context) {
...
function serializePadding(node) {
...
function serializeComponent(node) {
    ...
    const component = removeEmpty({
        ...
        mainComponent: mainComponent && typeof mainComponent === "object"
            ? removeEmpty({
                id: toPlainValue(mainComponent.id),
                name: toPlainValue(mainComponent.name),
                key: toPlainValue(mainComponent.key)
            })
            : undefined
    });
    return Object.keys(component).length > 0 ? component : undefined;
}
function serializeExportHints(node) {
...
function guessFlutterWidget(node, layoutMode, children) {
    if (node.type === "TEXT") {
        return "Text";
    }
...
function serializeBounds(bounds, rootBox) {
    if (!bounds) {
        return undefined;
    }
    const absolute = {
...
async function maybeReportNodeProgress(node, context) {
...
function postStatus(message, tone = "info") {
    postToUi({
        type: "status",
        message,
        tone
...
function readPlain(source, key) {
    return toPlainValue(readUnknown(source, key));
}
function readUnknown(source, key) {
    try {
